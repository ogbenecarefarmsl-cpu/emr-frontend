...
export function useOrderLab() {
  ...
  return useMutation({
    mutationFn: async (visitId: string) => {
      return await visitsAPI.orderLab(visitId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['visits'] });
    },
...
export function usePrescribeMedication() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (visitId: string) => {
      return await visitsAPI.prescribeMedication(visitId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['visits'] });
    },
...
export function useMarkLabPaid() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (visitId: string) => {
      return await visitsAPI.markLabPaid(visitId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['visits'] });
...
export function useMarkPharmacyPaid() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (visitId: string) => {
      return await visitsAPI.markPharmacyPaid(visitId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['visits'] });
      queryClient.invalidateQueries({ queryKey: ['orders'] });
